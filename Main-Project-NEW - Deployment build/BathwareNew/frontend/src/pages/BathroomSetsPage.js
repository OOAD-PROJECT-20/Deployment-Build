...
const BathroomSetsPage = ({ onBack, onAddToCart, cartCount = 0, isAuthenticated = true }) => {
  const navigate = useNavigate();
  const [bathroomSetProducts, setBathroomSetProducts] = useState([]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Filter products by "Bathroom Sets" category - use database imageUrl as-is
      const filteredProducts = response.data.filter(
        product => product.category?.name === 'Bathroom Sets'
      );
      setBathroomSetProducts(filteredProducts);
    } catch (error) {
  ...
  const getSortedProducts = () => {
    const sorted = [...bathroomSetProducts];
    switch (sortBy) {
      case 'price-low':
      ...
      <section className="shop-hero">
        ...
        <div className="shop-hero-content">
          <h1 className="shop-hero-title">
            Bathroom Sets
            <span className="shop-hero-gradient">Complete Solutions</span>
          </h1>
          <p className="shop-hero-description">
            Complete shower systems and bathroom packages. Transform your bathroom with our premium collection of complete bathroom solutions.
          </p>
        </div>
...
export default BathroomSetsPage;
