...
const WaterClosetsPage = ({ onBack, onAddToCart, cartCount = 0, isAuthenticated = true }) => {
  const navigate = useNavigate();
  const [waterClosetProducts, setWaterClosetProducts] = useState([]);
  ...
  const fetchProducts = async () => {
    try {
      ...
      setWaterClosetProducts(filteredProducts);
    } catch (error) {
  ...
  const getSortedProducts = () => {
    const sorted = [...waterClosetProducts];
    switch (sortBy) {
      case 'price-low':
      ...
      <section className="shop-hero">
        ...
        <div className="shop-hero-content">
          <h1 className="shop-hero-title">
            Water Closets
            <span className="shop-hero-gradient">Premium Collection</span>
          </h1>
          <p className="shop-hero-description">
            Premium toilets and commode systems for modern bathrooms. Choose from our extensive collection of high-quality water closets.
          </p>
        </div>
...
export default WaterClosetsPage;
