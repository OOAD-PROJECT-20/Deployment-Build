...
-- =====================================================
-- SAMPLE DATA FOR CART, QUOTATION, QUOTATION_ITEMS, ORDERS
-- =====================================================

-- 🛒 CART TABLE (user_id must match existing user.userId, product_id from products)
...
-- 🧾 QUOTATION TABLE (customer_id refers to user.userId)
INSERT INTO quotation (customer_id, total_price, qname, address, qnumber, qstatus)
VALUES
...
-- 🧩 QUOTATION_ITEMS TABLE (quotation_id, product_id, quantity, price)
INSERT INTO quotation_items (quotation_id, product_id, quantity, price)
VALUES
    (1, 1, 1, 45000.00),  -- Smart Toilet Commode
    (1, 5, 2, 18750.00),  -- Ceramic Wash Basin
    (2, 10, 1, 25000.00), -- Rain Shower System
    (2, 4, 1, 28000.00),  -- Close Coupled Toilet
    (3, 6, 1, 28000.00);  -- Granite Counter Basin

-- 📦 ORDERS TABLE (quotation_id, customer_id, total_amount, payment_slip, payment_status, deliver_status)
INSERT INTO orders (quotation_id, customer_id, total_amount, payment_slip, payment_status, deliver_status)
VALUES
...
CREATE TABLE support ( ticket_id BIGINT AUTO_INCREMENT PRIMARY KEY, user_id BIGINT NOT NULL, support_type VARCHAR(50) NOT NULL, description VARCHAR(255), remark VARCHAR(500), status VARCHAR(100) );
INSERT INTO support (user_id, support_type, description, remark, status) VALUES
                                                                             (1, 'Warranty', 'Leaking faucet in bathroom', 'Customer contacted, awaiting parts', 'Pending'),
                                                                             (2, 'Product Inquiry', 'Question about installation of basin', '', 'Pending'),
                                                                             (3, 'Complaint', 'Missing parts in order #1023', 'Investigating with warehouse', 'Pending'),
                                                                             (1, 'Warranty', 'Broken flush handle on water closet', '', 'Pending'),
                                                                             (4, 'General', 'Request for product catalog', 'Sent catalog via email', 'Solved'),
                                                                             (2, 'Warranty', 'Cracked ceramic on bathroom set', '', 'Pending');
SHOW TABLES;
